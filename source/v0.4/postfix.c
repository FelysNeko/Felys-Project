...
double rpncalc(char *, lyr *);
void rpnstd(char *, char *);
void rpncvt(container *, char *);
...
double rpnexpr(char *expr, lyr *base){
    ...
    if (write == TRUE){
        varwrite(base->next, name, result);
    }
...
double rpncalc(char *expr, lyr *base){
    char stdexpr[LEN] = {0};
    rpnstd(expr, stdexpr);
    ...
    for (int i=0; rpn[i][0]; ++i){
        ...
        if (letter(index)){
            stkpush(&calc, varget(base->next, rpn[i]));
        }else if (!token(index)){  
...
void rpncvt(container *rpn, char *expr){
...
void rpnstd(char *expr, char *target){
    char temp[LEN] = {0};
