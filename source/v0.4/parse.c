#define PARSLIB
#include "header.h"

typedef struct Command{
    struct Command *next;
    struct Command *jump;
    char *expr;
    char *type;
    int index;
    int scope;
}cmd;


int loadkw(int, char *, char *);
int tab(char *);
cmd *dtoc(double );
int empty(char *);

void cmdinit(cmd *);
void cmdlist(cmd *);
void cmdcheck(cmd *);
cmd *cmdread(char *, cmd *, int);
void cmdparse(container, cmd *);


cmd *cmdgoto(cmd *line, int i){
    for (; line!=NULL; line=line->next){
        if (line->index == i){
            return line;
        }
    }
    error("LineNotFound", -1);
    return NULL;
}


void cmdinit(cmd *home){
    home->expr = "NO EXPR HERE";
    home->index = 0;
    home->jump = NULL;
    home->scope = 0;
    home->type = "START";
}


void cmdparse(container raw, cmd *line){
    cmd * const home = line;

    for (int i=0; raw[i][0]!=0; ++i){
        if (!empty(raw[i])){
            line = cmdread(strstrip(raw[i], '\n'), line, i);
        }
    }

    cmdcheck(home->next);

    stack end; stkinit(&end);
    stack skip; stkinit(&skip);

    for (line=home->next; line!=NULL; line=line->next){
        cmd *next = line->next;
        cmd *top = dtoc(stktop(&end));
        cmd *jump = dtoc(stktop(&skip));

        if (top!=NULL && strmatch(top->type, WHILE) && top->scope==next->scope){
            line->jump = top;
            top->jump = next;
            stkpop(&end);
        }else if (top!=NULL && (strmatch(line->type, ELIF) || strmatch(line->type, ELSE)) && top->scope==line->scope){
            top->jump = line;
            stkpop(&end);
        }

        if (!strmatch(line->type, ELIF) && !strmatch(line->type, ELSE)){
            while (skip.top>-1 && jump!=NULL && line->scope==jump->scope-1){
                dtoc(stkpop(&skip))->jump = line;
            }
        }
        
        if (strmatch(line->type, IF) || strmatch(line->type, ELIF) || strmatch(line->type, WHILE)){
            stkpush(&end, (long)line);
        } else if (next!=NULL && (strmatch(next->type, ELIF) || strmatch(next->type, ELSE))){
            stkpush(&skip, (long)line);
        }
    }
}


void cmdcheck(cmd *line){
    for (; line!=NULL; line=line->next){
        char *type = line->type;
        if (strmatch(type, IF) || strmatch(type, ELIF) || strmatch(type, ELSE) || strmatch(type, WHILE)){
            if (line->next->scope-1 != line->scope){
                error("IncorrectTabs", line->index+1);
            }
        }
    }
}


cmd *cmdread(char *str, cmd *prev, int index){
    cmd *line = (cmd *)malloc(sizeof(cmd));
    prev->next = line;
    line->index = index+1;
    line->next = NULL;


    if ((line->scope = tab(str)) == -1){
        error("IncorrectTabs", index+1);
    }

    for (int diff=prev->scope-line->scope; diff>0; --diff){
        prev->next = (cmd *)malloc(sizeof(cmd));
        prev = prev->next;
        prev->scope = line->scope + diff;
        prev->type = NONE;
        prev->expr = NONE;
        line->jump = NULL;
        prev->index = -1;
        prev->next = line;
    }

    char keyword[LEN] = {0};
    int end = loadkw(line->scope*4, str, keyword);
    line->expr = str+end;

    if (strmatch(keyword, IF)){
        line->type = IF;
    } else if(strmatch(keyword, ELIF)){
        line->type = ELIF;
    } else if(strmatch(keyword, ELSE)){
        line->type = ELSE;
    } else if(strmatch(keyword, WHILE)){
        line->type = WHILE;
    } else if(strmatch(keyword, EXIT)){
        line->type = EXIT;
    } else if(strmatch(keyword, GOTO)){
        line->type = GOTO;
    } else{
        line->type = NONE;
        line->expr = str+line->scope*4;
    }

    return line;
}


void cmdlist(cmd *line){
    while (line!=NULL){
        printf("[%d]\t<%p> [%d] ", line->index, line, line->scope);
        printf("%8s (%s)", line->type, line->expr);
        if (line->jump != NULL){
            printf(" -> <%p>", line->jump);
        }
        putchar('\n');
        line = line->next;
    }
}


int loadkw(int start, char *str, char *target){
    int count = 0;
    for (; str[start+count]!=' ' && str[start+count]!=0; ++count){
        target[count] = str[start+count];
    }
    return start+count;
}


int tab(char *str){
    int count = 0;
    for (; str[count]==' '; ++count);

    int remainder = count%4;
    if (!remainder){
        return count/4;
    } else{
        return -1;
    }
}


cmd *dtoc(double x){
    return (cmd *)(long)x;
}


int empty(char *str){
    for (; *str==' ' || *str=='\n'; ++str);
    return *str==0 ? 1 : 0;
}
